...
void loop()
{
  ...
  if (playflag == 0)
  {
    ...
    {
     sflag = sflag - 1 < 0 ? numCh - 1 : sflag - 1;
    URL = arrayURL[sflag];
    station = arrayStation[sflag];
    M5.Lcd.fillScreen(BLACK);
    M5.Lcd.setTextSize(1);
  ...
  if (playflag == 1)
  {
    ...
    if (M5.Axp.GetBtnPress() == 2)
    {
      fgain = fgain - 1.0 < 0 ? fgain : fgain - 1.0;
      out->SetGain(fgain * 0.05 + 0.2);
      M5.Lcd.fillRect(109, 0, 160, 21, BLACK);
      M5.Lcd.fillTriangle(109, 20, 109 + (5 * fgain), 20, 109 + (5 * fgain), 20 - (2 * fgain), BLUE);
      Serial.printf("STATUS(Gain) %f \n", fgain * 0.05);
...
void StartPlaying()
{
  ...
  //  out->SetGain(0.3);
  out->SetGain(fgain * 0.05 + 0.2);
  M5.Lcd.fillTriangle(109, 20, 109 + (3 * fgain), 20, 109 + (3 * fgain), 20 - (2 * fgain), BLUE);
